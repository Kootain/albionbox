...
- [ ] Task 2: 集成图像识别组件
  - [ ] SubTask 2.1: 复用 `packages/shared/src/utils/api_image.ts` 的识别方法（或抽一层 wrapper）
  - [ ] SubTask 2.2: 定义“关键字段非空”判定规则（victimName/timestamp）
  - [ ] SubTask 2.3: 增加必要的环境变量约定（ARK_API_KEY、MODEL_ID 等）并更新 consumer-worker 配置说明

- [ ] Task 3: 调用 apps/api 创建 regear_apply
  - [ ] SubTask 3.1: consumer-worker 内新增 `API_BASE_URL` 与 `INTERNAL_API_TOKEN` 环境变量读取
  - [ ] SubTask 3.2: 调用 `POST /regear_applies` 写入申请（映射 msg 字段 + applyDetail）
  - [ ] SubTask 3.3:（可选）通过 msgId 做幂等：若已创建过则跳过
...
- [ ] Task 5: 验证与最小联调
  - [ ] SubTask 5.1: 为 consumer-worker 增加一个本地可运行的事件样例（单元测试或脚本）
  - [ ] SubTask 5.2: 本地运行 tsc，确保两个 apps 都无类型错误
