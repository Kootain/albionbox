...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [guildsRes, sessionsRes] = await Promise.all([
          api.guilds.$get(),
          api.regear.sessions.$get()
        ]);
        
        if (guildsRes.ok && sessionsRes.ok) {
          ...
          setStats({
            ...
            requests: 0,
            pending: guilds.filter((g: any) => g.status === 'pending').length
          });
        }
      } catch (error) {
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden bg-black-card border border-black-border rounded-3xl p-8 md:p-12 shadow-2xl">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          >
            <span className="text-gold font-black uppercase tracking-[0.3em] text-[10px] mb-4 block">Welcome Back</span>
            <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tighter uppercase leading-none mb-6">
              {t('dashboard.welcome')}, <span className="text-gold">{user?.email?.split('@')[0]}</span>
            </h1>
