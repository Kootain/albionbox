...
export const killEventJsonSchema = {
  type: 'object',
  properties: {
    victimName: { type: 'string', description: 'Name of the victim. on the left. Must be a single token with only letters and digits (no spaces, no punctuation).' },
    ...
    killerName: { type: 'string', description: 'Name of the killer. on the right. Must be a single token with only letters and digits (no spaces, no punctuation).' },
